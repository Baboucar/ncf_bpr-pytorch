...
parser.add_argument("--hidden_layer", type=list, default=[64, 32, 16], help="dimension of each hidden layer")
...
parser.add_argument("--data_path", type=str, default="/home/user/PycharmProjects/ncf-pytorch/data")
parser.add_argument("--model_path", type=str, default="/home/user/PycharmProjects/ncf-pytorch/model")
...
class BPRLoss(nn.Module):
    ...
    def forward(self, pos_preds, neg_preds):
      #  print("a",pos_preds.shape)
       # print("b",neg_preds.shape)
...
class MLP(nn.Module):
    ...
    def forward(self, user, item):
        ...
        # if is_pretrain:
        #     prediction = torch.sum(torch.mul(embed_user, embed_item), dim=1)
        # else:
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        for user, item, label in train_loader:
            ...
            model.zero_grad()
            # if epoch < 3:
            #     pos_preds = model(user, item, True)
            #     neg_preds = model(user, label, True)

            pos_preds = model(user, item)
            neg_preds = model(user, label)
        ...
        if epoch % 2 == 0 and epoch != 0:  # 50
            writer.add_scalar('NDCG@MLP', NDCG, epoch)
